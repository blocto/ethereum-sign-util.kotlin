 ...
 */
enum class SignTypedDataVersion {
    V1,
    V3,
    V4,
}

typealias TypedDataV1 = String

@Serializable data class MessageTypeProperty(val name: String, val type: String)
    ...
    companion object {
        ...
        private fun validateVersion(
            ...
            if (!SignTypedDataVersion.values().contains(version)) {
                ...
                throw Error("SignTypedDataVersion not allowed: '$version'. " +
        ...
        private fun solidityHexValue(type: String, value: Any): ByteArray = when {
            ...
            type == "string" -> BytesETHType(value.toString().toByteArray(),
                BytesTypeParams(32)).paddedValue
            ...
            type.startsWith("bytes") -> BytesETHType((value as ByteArray),
                BytesTypeParams(32)).toKotlinType()
            type.startsWith("uint") -> readNumber(rawNumber = value,
                creator = {
                    UIntETHType.ofNativeKotlinType(it,
                        BitsTypeParams(type.extractPrefixedNumber("uint",
                            INT_BITS_CONSTRAINT))
                    )
                }).paddedValue
            type.startsWith("int") -> readNumber(rawNumber = value,
                creator = {
                    IntETHType.ofNativeKotlinType(it,
                        BitsTypeParams(type.extractPrefixedNumber("int",
                            INT_BITS_CONSTRAINT))
                    )
        ...
        fun signTypedData(
            ...
            val messageHash = when (version) {
                SignTypedDataVersion.V1 -> _typedSignatureHash(json.decodeFromString(data))
                else -> eip712Hash(data)
            }
            val ecKeyPair = PrivateKey(privateKey).toECKeyPair()
            val sig = signMessageHash(messageHash, ecKeyPair)
            return "0x${sig.toHex()}"
        }
        ...
        private fun encodeField(
            ...
            when {
                ...
                type == "bytes" -> {
                    return BytesETHType((value as ByteArray).keccak(), BytesTypeParams(32))
                }
                type == "string" -> {
            ...
            if (type.lastIndexOf("]") == type.length - 1) {
                ...
                return BytesETHType(encodeTypes(*typeValuePairs.toTypedArray()).keccak(),
                    BytesTypeParams(32))
            }
            ...
            return when {
                ...
                type == "string" -> BytesETHType((value as String).toByteArray().keccak(),
                    BytesTypeParams(32))
                type.startsWith(prefix = "bytes") -> BytesETHType.ofNativeKotlinType(HexString(
                    value as String).hexToByteArray(),
                    BytesTypeParams((type.extractPrefixedNumber("bytes",
        ...
        private fun encodeData(
            ...
            val typeHash = typeHash(typeSpec, typeName).joinToString(separator = "")
                .toByteArray(charset = Charsets.UTF_8).keccak()
        ...
        private fun typeHash(
            ...
            val encodedStruct = types.joinToString(separator = ",",
                prefix = "$typeName(",
                postfix = ")",
                transform = { (name, type) -> "${type} $name" })
            val structParams = types.asSequence()
                .filterNot { typeSpec[it.type] == null }
                .map { (_, type) -> typeHash(typeSpec, type) }
